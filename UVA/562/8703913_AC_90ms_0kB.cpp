...
int coins[120], dp[120][MX];

int ff( int i, int vlu, int n, int sm ) {
        if( i == n ) {
                return abs( (sm - vlu) - vlu );
        }
        if( dp[i][vlu] != -1 ) return dp[i][vlu];
        return dp[i][vlu] = min( ff( i+1, vlu + coins[i], n, sm ), ff( i+1, vlu, n, sm ) );
}

int main() {
        ///write;
        FasterIO
        int cases, caseno = 0, n, sm;
        cin >> cases;
        while( cases-- ) {
                cin >> n;
                sm = 0;
