...
int ff( int vlu, int i, int n, int mod ) {
        if( i == n ) {
                if( (vlu+mod) % mod == 0 ) return 1;
                return 0;
        }
        if( dp[vlu][i] != -1 ) return dp[vlu][i];
        return dp[vlu][i] = max( ff( (vlu + ar[i]%mod + mod) % mod, i+1, n, mod ), ff( (vlu - ar[i]%mod + mod) % mod, i+1, n, mod ) );
}
...
int main() {
        ...
        while( cases-- ) {
                ...
                if( ff(0, 0, n, k) ) cout << "Divisible" << endl;
